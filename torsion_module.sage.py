...
# This file was *autogenerated* from the file torsion_module.sage
from sage.all_cmdline import *   # import sage library

_sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_3 = Integer(3); _sage_const_4 = Integer(4); _sage_const_16 = Integer(16); _sage_const_32 = Integer(32); _sage_const_8 = Integer(8)# Stage 2: compute J[2] = H_1(X, F_2) as an F_2[G]-module for a Galois 2-group
...
def analyze(G, s0, s1, verbose=True):
    ...
    s1inv = s1**(-_sage_const_1 )
    R1inv = right_perm(elts, idx, s1inv)

    V = VectorSpace(F2, _sage_const_2 *d)          # e0_g -> g ,  e1_g -> d+g
    C0 = VectorSpace(F2, d)
    # boundary d1 as 2d x d (rows = edges); build kernel of the map C_1 -> C_0
    rows = []
    for g in range(d):                # e0_g
        r = [_sage_const_0 ]*d; r[g] += _sage_const_1 ; r[R0[g]] += _sage_const_1 ; rows.append(r)
    for g in range(d):                # e1_g
        r = [_sage_const_0 ]*d; r[g] += _sage_const_1 ; r[R1[g]] += _sage_const_1 ; rows.append(r)
    ...
    assert Z.dimension() == d + _sage_const_1 , (Z.dimension(), d+_sage_const_1 )

    # relations
    rels = []
    for o in orbits_under(R0):        # L0
        v = [_sage_const_0 ]*(_sage_const_2 *d)
        for g in o: v[g] += _sage_const_1 
        rels.append(v)
    for o in orbits_under(R1):        # L1
        v = [_sage_const_0 ]*(_sage_const_2 *d)
        for g in o: v[d+g] += _sage_const_1 
        rels.append(v)
    for o in orbits_under(Rinf):      # Linf : sum_k e1_{g sinf^k s1^-1} + e0_{g sinf^{k+1}}
        g0 = o[_sage_const_0 ]; v = [_sage_const_0 ]*(_sage_const_2 *d); j = g0
        for _ in range(len(o)):
            v[d + R1inv[j]] += _sage_const_1       # e1_{ j * s1^{-1} }
            v[Rinf[j]] += _sage_const_1            # e0_{ j * sinf }
            j = Rinf[j]
        rels.append(v)
    ...
    twog = len(basisH)
    assert twog % _sage_const_2  == _sage_const_0 , twog
    genus = twog // _sage_const_2 
    ...
    def matfn(g):
        ...
        for zb in basisH:             # zb is a V-vector (a cycle in Zs)
            y = [_sage_const_0 ]*(_sage_const_2 *d)
            for g_ in range(d):
                if zb[g_]:   y[Lp[g_]] += _sage_const_1      # e0_g -> e0_{h g}
                if zb[d+g_]: y[d+Lp[g_]] += _sage_const_1    # e1_g -> e1_{h g}
...
def analyze_nongalois(G, s0, s1, H):
    ...
    return dimX // _sage_const_2 , dimX, A

def gl_solvable(a, q):
    # GL_a(q) solvable iff a==1, or (a==2 and q in {2,3}). (q is a power of 2 here.)
    if a == _sage_const_1 : return True
    if a == _sage_const_2 : return q in (_sage_const_2 , _sage_const_3 )
    return False
...
def commutant_basis(n, Ms):
    ...
    for P in Ms:
        for a in range(n):
            for b in range(n):
                row = [_sage_const_0 ]*(n*n)              # coeff of X[i,j] in (XP-PX)[a,b]
                for i in range(n):
                    if P[a][i]:
                        row[i*n + b] += _sage_const_1     # -(P X)[a,b] term: sum_i P[a,i] X[i,b]
                for j in range(n):
                    if P[j][b]:
                        row[a*n + j] += _sage_const_1     # (X P)[a,b] term: sum_j X[a,j] P[j,b]
                rows.append(row)
    C = Matrix(F2, rows) if rows else Matrix(F2, _sage_const_0 , n*n)
    K = C.right_kernel()
...
def centralizer_solvable(dim, Ms):
    ...
    A = libgap.AlgebraWithOne(libgap.GF(_sage_const_2 ), gb)
...
def _min_faithful_degree(G):
    ...
    for k in range(int(libgap.Length(maxs))):
        M = maxs[k]
        if int(libgap.Order(libgap.Core(G, M))) == _sage_const_1 :   # faithful coset action
...
def block_status(a, q):
    ...
    if not (a >= _sage_const_3  or (a == _sage_const_2  and q >= _sage_const_4 )):
    ...
    name = f"PSL_{a}(F_{q})"
    if a == _sage_const_2  and q == _sage_const_16 :
...
def nongalois_realizability_scan(degrees):
    ...
    dead = _sage_const_0 ; live = []; seen_status = {}
    for order in degrees:
        ...
        for i in range(_sage_const_1 , n + _sage_const_1 ):
            ...
            for k in range(int(libgap.Length(ccs))):
                ...
                if oH == _sage_const_1  or oH == order: continue
                if int(libgap.Order(libgap.Core(G, H))) != _sage_const_1 : continue
                Hreps.append(H)
            ...
            for H in Hreps:
                ...
                for h in quos:
                    ...
                    if gX < _sage_const_2 : continue
                    solvC, blocks, _ = centralizer_solvable(dim, Amats)
                    if solvC:
                        dead += _sage_const_1 ; continue
                    verdicts = [block_status(a, q) for (a, q) in blocks]
                    ns = [v for v in verdicts if v[_sage_const_0 ] != 'solvable']
                    worst = ('NO' if all(v[_sage_const_0 ] == 'NO' for v in ns) else
                             'FRONTIER' if any(v[_sage_const_0 ] == 'FRONTIER' for v in ns) else 'UNKNOWN')
                    if worst == 'NO':
                        dead += _sage_const_1 
                        seen_status[('DEAD', gX, tuple(sorted(v[_sage_const_1 ] for v in ns)))] =                             seen_status.get(('DEAD', gX, tuple(sorted(v[_sage_const_1 ] for v in ns))), _sage_const_0 ) + _sage_const_1 
                    else:
                        tag = f"g{gX} d={d} G=[{order},{i}]{struct} |Aut|={autX}"
                        live.append((worst, tag, [v[_sage_const_1 ] for v in ns]))
    print(f"\n==== REALIZABILITY FILTER ({sum(len(v) for v in [range(_sage_const_0 )]) or ''}) ====")
...
def scan(degrees):
    F = libgap.FreeGroup(_sage_const_2 ); fgens = libgap.GeneratorsOfGroup(F)
    nonsolv = []
    for d in degrees:
        ...
        for i in range(_sage_const_1 , n+_sage_const_1 ):
            ...
            for h in libgap.GQuotients(F, G):
                s0 = libgap.Image(h, fgens[_sage_const_0 ]); s1 = libgap.Image(h, fgens[_sage_const_1 ])
                genus, dim, Ms, _ = analyze(G, s0, s1)
                if genus < _sage_const_2 :
                    continue
                ntri += _sage_const_1 
...
if __name__ == "__main__":
    ...
    if argv and argv[_sage_const_0 ] == "ng":
        nongalois_scan([int(x) for x in argv[_sage_const_1 :]] or [_sage_const_16 , _sage_const_32 ])
        sys.exit(_sage_const_0 )
    if argv and argv[_sage_const_0 ] == "ngr":
        nongalois_realizability_scan([int(x) for x in argv[_sage_const_1 :]] or [_sage_const_16 , _sage_const_32 ])
        sys.exit(_sage_const_0 )
    ...
    if args:
        ...
        genus, dim, Ms, _ = analyze(G, gens[_sage_const_0 ], gens[_sage_const_1 ])
